...
let get_hotplug_extra_path (x: device) =
	sprintf "%s/hotplug-extra/%s/%d" (get_private_path x.frontend.domid) (string_of_kind x.backend.kind) x.backend.devid
...
let wait_for_plug ~xs (x: device) = 
  ...
  try
	...
	if use_new_mechanism then (
          let dev = sprintf "/devices/%s-%d-%d" (string_of_kind x.backend.kind) (x.frontend.domid) (x.frontend.devid) in
	  Udev.wait_for "add" dev 150.
	) else (
...
let mount_loopdev ~xs (x: device) file readonly =
	let path = get_hotplug_extra_path x ^ "/loop-device" in
...
let release_loop_devs ~xs (x: device) =
	let path = get_hotplug_extra_path x in
