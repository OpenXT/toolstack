...
CAMLprim value stub_xc_domain_set_xci_cpuid_signature(value handle, value domid, value on)
{
	CAMLparam3(handle, domid, on);
	int ret;
        int c_on = Bool_val(on);
	ret = xc_set_hvm_param(_H(handle), _D(domid), HVM_PARAM_XCI_CPUID_SIGNATURE, c_on);
 	if (ret < 0)
		failwith_xc(_H(handle));
	CAMLreturn(Val_unit);
}

CAMLprim value stub_xc_domain_set_cores_per_socket(value handle, value domid, value cores_per_socket)
{
    CAMLparam3(handle, domid, cores_per_socket);
    int rv;
    rv = xc_domain_set_cores_per_socket(_H(handle), _D(domid), Int_val(cores_per_socket));
    if (rv)
        failwith_xc(_H(handle));
    CAMLreturn(Val_unit);
}
...
CAMLprim value stub_xc_physdev_map_pirq(value xch, value domid, value pirq)
{
    ...
    int c_pirq = Int_val(pirq);
    int rv;
    rv = xc_physdev_map_pirq(_H(xch), _D(domid), c_pirq, &c_pirq);
    if (rv < 0)
        failwith_xc(_H(xch));
    CAMLreturn(Val_int(c_pirq));
}

CAMLprim value stub_xc_physdev_unmap_pirq(value xch, value domid, value pirq)
{
    CAMLparam3(xch, domid, pirq);
    int c_pirq = Int_val(pirq);
    int rv;
    rv = xc_physdev_unmap_pirq(_H(xch), _D(domid), Int_val(pirq));
    if (rv < 0)
        failwith_xc(_H(xch));
    CAMLreturn(Val_unit);
}
...
CAMLprim value stub_xc_domain_set_target(value xch, value domid, value target)
{
	CAMLparam3(xch, domid, target);
	int ret;
	ret = xc_domain_set_target(_H(xch), _D(domid), _D(target));
	if (ret < 0)
		failwith_xc(_H(xch));
	CAMLreturn(Val_unit);
}
...
{
	CAMLparam3(handle, domid, mode);
	int ret;

	ret = xc_set_hvm_param(_H(handle), _D(domid), HVM_PARAM_TIMER_MODE, (unsigned long) Int_val(mode));
 	if (ret < 0)
		failwith_xc(_H(handle));
	CAMLreturn(Val_unit);
}
 
CAMLprim value stub_xc_domain_set_hpet(value handle, value domid, value mode)
{
	CAMLparam3(handle, domid, mode);
	int ret;

	ret = xc_set_hvm_param(_H(handle), _D(domid), HVM_PARAM_HPET_ENABLED, (unsigned long) Int_val(mode));
	if (ret < 0)
		failwith_xc(_H(handle));
	CAMLreturn(Val_unit);
}

CAMLprim value stub_xc_domain_set_vpt_align(value handle, value domid, value mode)
{
	CAMLparam3(handle, domid, mode);
	int ret;

	ret = xc_set_hvm_param(_H(handle), _D(domid), HVM_PARAM_VPT_ALIGN, (unsigned long) Int_val(mode));
	if (ret < 0)
		failwith_xc(_H(handle));
	CAMLreturn(Val_unit);
}
...
CAMLprim value stub_xc_domain_send_s3resume(value handle, value domid)
{
	...
	xc_set_hvm_param(_H(handle), _D(domid), HVM_PARAM_ACPI_S_STATE, 0);
	CAMLreturn(Val_unit);
}
...
CAMLprim value stub_xc_domain_get_acpi_s_state(value handle, value domid)
{
	...
	ret = xc_get_hvm_param(_H(handle), _D(domid), HVM_PARAM_ACPI_S_STATE, &val);
 	if (ret != 0)
		failwith_xc(_H(handle));
	CAMLreturn(Val_int(val));
}

CAMLprim value stub_xc_hvm_check_pvdriver(value handle, value domid)
{
	CAMLparam2(handle, domid);
	int ret;
	unsigned long val;

	ret = xc_get_hvm_param(_H(handle), _D(domid), HVM_PARAM_CALLBACK_IRQ, &val);
 	if (ret != 0)
		failwith_xc(_H(handle));
	CAMLreturn(Val_bool(val ? 1 : 0));
}

CAMLprim value stub_xc_flask_load(value handle, value buffer)
{
	CAMLparam2(handle, buffer);
	int ret;
	ret = xc_flask_load(_H(handle), String_val(buffer), caml_string_length(buffer));
	if (ret < 0)
		failwith_xc(_H(handle));
	CAMLreturn(Val_unit);
}

CAMLprim value stub_xc_flask_context_to_sid(value handle, value context)
{
	CAMLparam2(handle, context);
	int ret;
	uint32_t sid;
        /* xc_flask_context_to_sid updates the buffer so we have to copy it here */
        char *buf = strdup( String_val(context) );

	ret = xc_flask_context_to_sid(_H(handle), buf, strlen(buf), &sid);
        free( buf );

	if (ret < 0)
		failwith_xc(_H(handle));
	CAMLreturn(caml_copy_int32(sid));
}

CAMLprim value stub_xc_flask_sid_to_context(value handle, value sid)
{
	CAMLparam2(handle, sid);
	int ret;
	char context[1024];

	ret = xc_flask_sid_to_context(_H(handle), Int32_val(sid), context, 1024);
	if (ret < 0)
		failwith_xc(_H(handle));
	CAMLreturn(caml_copy_string(context));
}

CAMLprim value stub_xc_flask_get_enforce(value handle)
{
	CAMLparam1(handle);
	int ret;

	ret = xc_flask_getenforce(_H(handle));
	if (ret < 0)
		failwith_xc(_H(handle));
	CAMLreturn(Val_bool(!!ret));
}

CAMLprim value stub_xc_flask_set_enforce(value handle, value mode)
{
	CAMLparam2(handle, mode);
	int ret;

	ret = xc_flask_setenforce(_H(handle), Bool_val(mode));
	if (ret < 0)
		failwith_xc(_H(handle));
	CAMLreturn(Val_unit);
}

CAMLprim value stub_xc_flask_add_pirq(value handle, value pirq, value context)
{
	CAMLparam3(handle, pirq, context);
	int ret;

	ret = xc_flask_add_pirq(_H(handle), Int_val(pirq), String_val(context));
	if (ret < 0)
		failwith_xc(_H(handle));
	CAMLreturn(Val_unit);
}

CAMLprim value stub_xc_flask_add_ioport(value handle, value low, value high, value context)
{
	CAMLparam4(handle, low, high, context);
	int ret;

	ret = xc_flask_add_ioport(_H(handle), Int64_val(low), Int64_val(high), String_val(context));
	if (ret < 0)
		failwith_xc(_H(handle));
	CAMLreturn(Val_unit);
}

CAMLprim value stub_xc_flask_add_iomem(value handle, value low, value high, value context)
{
	CAMLparam4(handle, low, high, context);
	int ret;

	ret = xc_flask_add_iomem(_H(handle), Int64_val(low), Int64_val(high), String_val(context));
	if (ret < 0)
		failwith_xc(_H(handle));
	CAMLreturn(Val_unit);
}

CAMLprim value stub_xc_flask_add_device(value handle, value device, value context)
{
	CAMLparam3(handle, device, context);
	int ret;

	ret = xc_flask_add_device(_H(handle), Int64_val(device), String_val(context));
	if (ret < 0)
		failwith_xc(_H(handle));
	CAMLreturn(Val_unit);
}

CAMLprim value stub_xc_flask_del_pirq(value handle, value pirq)
{
	CAMLparam2(handle, pirq);
	int ret;

	ret = xc_flask_del_pirq(_H(handle), Int_val(pirq));
	if (ret < 0)
		failwith_xc(_H(handle));
	CAMLreturn(Val_unit);
}

CAMLprim value stub_xc_flask_del_ioport(value handle, value low, value high)
{
	CAMLparam3(handle, low, high);
	int ret;

	ret = xc_flask_del_ioport(_H(handle), Int64_val(low), Int64_val(high));
	if (ret < 0)
		failwith_xc(_H(handle));
	CAMLreturn(Val_unit);
}

CAMLprim value stub_xc_flask_del_iomem(value handle, value low, value high)
{
	CAMLparam3(handle, low, high);
	int ret;

	ret = xc_flask_del_iomem(_H(handle), Int64_val(low), Int64_val(high));
	if (ret < 0)
		failwith_xc(_H(handle));
	CAMLreturn(Val_unit);
}

CAMLprim value stub_xc_flask_del_device(value handle, value device)
{
	CAMLparam2(handle, device);
	int ret;

	ret = xc_flask_del_device(_H(handle), Int64_val(device));
	if (ret < 0)
		failwith_xc(_H(handle));
	CAMLreturn(Val_unit);
}

CAMLprim value stub_xc_domain_disable_migrate(value handle, value domid)
{
	CAMLparam2(handle, domid);
	int ret;

	ret = xc_domain_disable_migrate(_H(handle), _D(domid));
	if (ret < 0)
		failwith_xc(_H(handle));
	CAMLreturn(Val_unit);
}
