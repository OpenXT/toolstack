...
(** Standalone wrapper process which safely closes fds before exec()ing another
    program *)
let close_and_exec_prg = "closeandexec"
let close_and_exec_path = "/opt/xensource/libexec/closeandexec"

exception Close_and_exec_binary_missing of string

let close_and_exec_cmdline ?(withpath=false) (fds: Unix.file_descr list) (cmd: string) (args: string list) = 
  (* Sanity check: make sure the close_and_exec binary exists *)
  if not withpath then (
    try Unix.access close_and_exec_path [ Unix.X_OK ] 
    with _ -> raise (Close_and_exec_binary_missing close_and_exec_path)
  );

  let fds = List.map (fun x -> string_of_int (Unixext.int_of_file_descr x)) fds in
  let closeandexec_argv =
    if withpath then
      close_and_exec_prg :: "--exec-with-path" :: fds
    else
      close_and_exec_path :: fds
    in
  closeandexec_argv @ ("--" :: cmd :: args) 
...
let fork_and_exec ?(withpath=false) ?(pre_exec=fun () -> ()) ?env (cmdline: string list) =
  ...
  if pid = 0 then begin
	...
	match env with
	| None -> (if withpath then Unix.execvp else Unix.execv) argv0 args
	| Some env -> (if withpath then Unix.execvpe else Unix.execve) argv0 args env
      with _ -> exit 1
  end else pid
...
let safe_close_and_exec ?withpath ?env (pre_exec: fd_operation list) (fds: Unix.file_descr list)
    (cmd: string) (args: string list) =
  let cmdline = close_and_exec_cmdline ?withpath fds cmd args in
  fork_and_exec ?withpath ~pre_exec:(fun () -> List.iter do_fd_operation pre_exec) ?env cmdline
...
(** Creates a temporary file and opens it for logging. The fd is passed to the function
    'f'. The logfile is guaranteed to be closed and unlinked afterwards. If the
    function 'f' throws an error then the log file contents are read in *)
let with_logfile_fd prefix f = 
...
let execute_command_get_output ?withpath ?env ?(cb_set=(fun _ -> ())) ?(cb_clear=(fun () -> ())) cmd args =
  ...
  let pid = ref 0 in
    ...
    (fun () ->
       ...
       (fun log_fd ->
	  pid := safe_close_and_exec ?withpath ?env
